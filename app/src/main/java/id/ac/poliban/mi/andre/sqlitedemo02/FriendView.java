package id.ac.poliban.mi.andre.sqlitedemo02;
...
import id.ac.poliban.mi.andre.sqlitedemo02.dao.impl.FriendDaoImplSQLite;
import id.ac.poliban.mi.andre.sqlitedemo02.domain.Friend;

import static id.ac.poliban.mi.andre.sqlitedemo02.MainActivity.EVENT_INSERT;
import static id.ac.poliban.mi.andre.sqlitedemo02.MainActivity.EVENT_UPDATE;
import static id.ac.poliban.mi.andre.sqlitedemo02.MainActivity.EVENT_VIEW;
...
public class FriendView extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btSimpan.setOnClickListener(v -> {
            ...
            if (getIntent().getIntExtra("event", -1) == EVENT_INSERT) {
                Friend friend = new Friend(etName.getText().toString(), etAddress.getText().toString(), etPhone1.getText().toString(), etPhone2.getText().toString(), etPhone3.getText().toString(), etEmail.getText().toString(), etHobbies.getText().toString());

                ds.insert(friend); //sisipkan object Friend             }             else if(getIntent().getIntExtra("event", -1)==EVENT_UPDATE) {                 Friend friend = new Friend(                         Integer.parseInt(etId.getText().toString()),                         etName.getText().toString(),                         etAddress.getText().toString(),                         etPhone1.getText().toString(),                         etPhone2.getText().toString(),                         etPhone3.getText().toString(),                         etEmail.getText().toString(),                         etHobbies.getText().toString());
    ...
    private boolean isValidate() {
        EditText[] ets = new EditText[]{etName, etEmail};
