...
public class MainActivity extends AppCompatActivity {
    ...
    private void tampilkanMenu(AdapterView<?> parent, View view, int position, long id) {
        ...
        new AlertDialog.Builder(this).setTitle("Action..").setIcon(android.R.drawable.ic_dialog_info).setItems(items, (dialog, which) -> {
            ...
            } else if (items[which].equals("Delete")) {                         //ambil data dari list
                Friend friend = (Friend) parent.getAdapter().getItem(position);
                ds.delete(friend.getId());
                data.remove(friend);
                adapter.notifyDataSetChanged();
            } else if (items[which].equals("Show Detail")) {                         //ambil data dari list
                Friend f = (Friend) parent.getAdapter().getItem(position);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        if (item.getItemId() == R.id.miAddAccount) {
            Intent intent = new Intent(this, FriendView.class);             //kirim pesan via intent bahwa user ingin insert data
